...
import { useDashboardConfig, PANEL_LABELS } from "@/hooks/use-dashboard-config";
import { METRICS, METRICS_BY_ID, type OverviewData, type CronRunsData } from "@/lib/dashboard-metrics";

// ── Icon map ─────────────────────────────────────────────────────────────────
const ICON: Record<string, (cls: string) => React.ReactNode> = {
  zap:            (c) => <Zap            className={cn("w-4 h-4", c)} />,
  star:           (c) => <Star           className={cn("w-4 h-4", c)} />,
  "check-square": (c) => <CheckSquare   className={cn("w-4 h-4", c)} />,
  brain:          (c) => <Brain          className={cn("w-4 h-4", c)} />,
  "calendar-clock":(c) => <CalendarClock className={cn("w-4 h-4", c)} />,
  bot:            (c) => <Bot            className={cn("w-4 h-4", c)} />,
  activity:       (c) => <Activity       className={cn("w-4 h-4", c)} />,
};
...
export default function OverviewPage() {
  ...
  const { config, setSlotMetric, toggleSlot, togglePanel, movePanel, reset } = useDashboardConfig();
  ...
  const { data: cronRuns } = useQuery<CronRunsData>({
    queryKey: ["cron-runs"],
  ...
  useEffect(() => {
    if (!data) return;
    if ((data as OverviewData & { noRuntime?: boolean }).noRuntime) {
      fetch("/api/runtime-sources/auto-setup")
        .then((r) => r.json())
        .then((res) => {
          if (res.provisioned) qc.invalidateQueries({ queryKey: ["overview"] });
          else if (res.reason !== "already_configured" && !data.gateway?.alive) router.push("/onboarding");
        })
        .catch(() => { if (!data.gateway?.alive) router.push("/onboarding"); });
    }
    const d = data as OverviewData & { runtimeSource?: { id: string; name: string; status: string; lastSyncAt: string | null } };
    if (d.runtimeSource) {
      setActiveRuntime({ id: d.runtimeSource.id, name: d.runtimeSource.name, gatewayUrl: "", status: d.runtimeSource.status });
  ...
  if (isLoading) return <div className="flex items-center justify-center h-64"><div className="text-sm text-zinc-500">Loading…</div></div>;
  if (error) return (
  ...
  const hiddenCount = config.slots.filter((s) => s.hidden).length
                    + config.panels.filter((p) => p.hidden).length;
  ...
  const panelMap: Record<string, React.ReactNode> = {
    agents: (
      <Panel key="agents" title="Agents" icon={<Bot className="w-3.5 h-3.5" />} href="/agents"
        ...
        {d.agentList.length === 0 && agents.length === 0 ? (
          <EmptyRow label="No agents configured" />
        ) : (
          (d.agentList.length > 0 ? d.agentList : agents).slice(0, 5).map((a, idx) => {
        ...
        {d.fallbacks.length > 0 && (
          <div className="px-4 py-2 flex items-center gap-1.5 border-t border-zinc-800/50">
            ...
            <span className="text-[10px] text-zinc-600 truncate">
              Fallbacks: {d.fallbacks.map((f) => f.split("/").pop()).join(" → ")}
            </span>
          </div>
        )}
      </Panel>
    ...
    tasks: (
      <Panel key="tasks" title="Active tasks" icon={<CheckSquare className="w-3.5 h-3.5" />} href="/tasks"
        badge={d.taskStats.blocked > 0 ? `${d.taskStats.blocked} blocked` : undefined}
        badgeClass="text-red-400 border-red-900/50">
        {recentTasks.length === 0 ? <EmptyRow label="No pending tasks" /> : (
          recentTasks.map((t) => (
        ...
        <div className="px-4 py-2 border-t border-zinc-800/50 flex items-center gap-3 text-[10px] text-zinc-600">
          <span>{d.taskStats.pending} pending</span><span className="text-zinc-700">·</span>
          <span>{d.taskStats.inProgress} in progress</span><span className="text-zinc-700">·</span>
          <span>{d.taskStats.done} done</span>
        </div>
      </Panel>
    ),
    schedules: (
      <Panel key="schedules" title="Upcoming schedules" icon={<CalendarClock className="w-3.5 h-3.5" />} href="/schedules"
        ...
        {cronJobs.length === 0 ? <EmptyRow label="No cron jobs configured" /> : (() => {
          const sorted = [...cronJobs].sort((a, b) => {
            if (!a.enabled && b.enabled) return 1;
            if (a.enabled && !b.enabled) return -1;
            if (!a.nextRunAt && !b.nextRunAt) return 0;
            if (!a.nextRunAt) return 1; if (!b.nextRunAt) return -1;
          ...
          return sorted.slice(0, 5).map((j) => {
            ...
            const nextLabel = nextMs !== null
              ...
              : `${Math.round(nextMs / 86400_000)}d` : null;
              ...
              <div key={j.id} className="flex items-center gap-2.5 px-4 py-2.5 border-b border-zinc-800/50 last:border-0">
                ...
                {j.enabled && nextLabel ? (
                  <span className={cn("text-[10px] shrink-0 font-mono", isSoon ? "text-amber-400" : "text-zinc-500")}>
                    {isSoon && <Clock className="w-2.5 h-2.5 inline mr-0.5 -mt-0.5" />}in {nextLabel}
                  </span>
                ) : j.enabled ? (
    ...
    activity: (
      <Panel key="activity" title="Recent activity" icon={<Activity className="w-3.5 h-3.5" />} href="/activity">
        ...
        {recentActivity.length === 0 ? <EmptyRow label="No activity yet" /> : (
  ...
  const visibleSlots  = config.slots.filter((s) => !s.hidden);
  const visiblePanels = config.panels.filter((p) => !p.hidden);
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-zinc-500 mt-0.5">
            {d.runtimeSource
              ? <>Mission control · synced {timeAgo(d.runtimeSource.lastSyncAt)}</>
      ...
      {/* ── Customize panel ─────────────────────────────────────────────────── */}
      {customizing && (
        <div className="bg-zinc-900 border border-violet-800/40 rounded-xl p-4 space-y-5">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-zinc-300">Customize dashboard</span>
            <div className="flex items-center gap-2">
              <button onClick={reset} className="flex items-center gap-1.5 text-[10px] text-zinc-600 hover:text-zinc-400 transition-colors">
          ...
          {/* Stat card slots */}
          <div>
            <p className="text-[10px] uppercase tracking-wider text-zinc-600 mb-3">Stat cards — choose metric per slot</p>
              ...
              {config.slots.map((slot, idx) => (
                <div
                  ...
                  className={cn(
                    ...
                    slot.hidden ? "border-zinc-800 bg-zinc-900/40 opacity-60" : "border-zinc-700 bg-zinc-800/30"
                  )}
                >
                  <span className="text-[10px] text-zinc-600 shrink-0 w-12">Slot {idx + 1}</span>

                  <select
                    value={slot.metricId}
                    onChange={(e) => setSlotMetric(idx, e.target.value)}
                    disabled={slot.hidden}
                    className="flex-1 bg-zinc-900 border border-zinc-700 rounded px-2 py-1 text-xs text-zinc-300 outline-none focus:border-violet-500 disabled:opacity-50"
                  >
                    {METRICS.map((m) => (
                  ...
                  <button
                    ...
                    className={cn("shrink-0 transition-colors", slot.hidden ? "text-zinc-700 hover:text-zinc-400" : "text-zinc-400 hover:text-zinc-200")}
                    title={slot.hidden ? "Show slot" : "Hide slot"}
                  >
                    {slot.hidden ? <EyeOff className="w-3.5 h-3.5" /> : <Eye className="w-3.5 h-3.5" />}
                  </button>
                </div>
              ))}
            </div>
          ...
          {/* Panels */}
          <div>
            <p className="text-[10px] uppercase tracking-wider text-zinc-600 mb-3">Panels</p>
            <div className="space-y-1.5">
              {config.panels.map((w, idx) => (
                <div key={w.id} className={cn(
                  "flex items-center gap-3 px-3 py-2 rounded-lg border transition-colors",
                  w.hidden ? "border-zinc-800 bg-zinc-900/40" : "border-zinc-700 bg-zinc-800/30"
                )}>
                  <button onClick={() => togglePanel(w.id)} className={cn("shrink-0 transition-colors", w.hidden ? "text-zinc-700 hover:text-zinc-500" : "text-zinc-400 hover:text-zinc-200")} title={w.hidden ? "Show" : "Hide"}>
                    {w.hidden ? <EyeOff className="w-3.5 h-3.5" /> : <Eye className="w-3.5 h-3.5" />}
                  </button>
                  <span className={cn("text-xs flex-1", w.hidden ? "text-zinc-600" : "text-zinc-300")}>{PANEL_LABELS[w.id]}</span>
                  <div className="flex items-center gap-0.5 shrink-0">
                    <button onClick={() => movePanel(w.id, "up")} disabled={idx === 0} className="p-1 text-zinc-700 hover:text-zinc-400 disabled:opacity-30 disabled:cursor-not-allowed transition-colors">
                      <ChevronUp className="w-3.5 h-3.5" />
                    </button>
                    <button onClick={() => movePanel(w.id, "down")} disabled={idx === config.panels.length - 1} className="p-1 text-zinc-700 hover:text-zinc-400 disabled:opacity-30 disabled:cursor-not-allowed transition-colors">
      ...
      {!alertDismissed && cronRuns && cronRuns.failedCount > 0 && (
        <div className="flex items-center gap-3 bg-red-950/30 border border-red-800/50 rounded-lg px-4 py-2.5">
          ...
          <Link href="/schedules" className="text-[10px] text-red-400 hover:text-red-300 underline shrink-0">View schedules</Link>
      ...
      {visibleSlots.length > 0 && (
        <div className={cn(
          "grid gap-3",
          visibleSlots.length >= 4 ? "grid-cols-2 md:grid-cols-4"
          : visibleSlots.length === 3 ? "grid-cols-2 md:grid-cols-3"
          : visibleSlots.length === 2 ? "grid-cols-2"
          : "grid-cols-1"
        )}>
          {visibleSlots.map((slot, idx) => {
            const metric = METRICS_BY_ID[slot.metricId];
            if (!metric) return null;
            const result = metric.compute(data, cronRuns, gatewayFailCount);
            return (
              <StatCard
                key={idx}
                icon={ICON[metric.iconKey]?.(result.valueClass) ?? <Zap className={cn("w-4 h-4", result.valueClass)} />}
                label={metric.cardLabel}
                value={result.value}
                valueClass={result.valueClass}
                sub={result.sub}
                href={result.href}
              />
            );
    ...
    <div className="bg-zinc-900 border border-zinc-800 rounded-lg overflow-hidden">
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-zinc-800">
        <div className="flex items-center gap-2 text-xs font-medium text-zinc-400">
          {icon}{title}
          {badge && <span className={cn("text-[9px] border px-1.5 py-0.5 rounded-full", badgeClass ?? "text-zinc-500 border-zinc-700")}>{badge}</span>}
        </div>
